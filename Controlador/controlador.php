...
class Controlador
{
    ...
    public function guardarMedico($doc, $nom, $ape)
    {
        $gestorCita = new GestorCita();
        $registro = $gestorCita->agregarMedico(new Medico($doc, $nom, $ape));
        if ($registro > 0) {
            $_SESSION['mensaje'] = "Se insertó el médico con éxito";
        } else {
            $_SESSION['mensaje'] = "Error al grabar el médico";
        }
    ...
    public function agregarMedico($doc, $nom, $ape)
    {
        ...
        $gestorCita = new GestorCita();
        $registro = $gestorCita->agregarMedico($medico);
        if ($registro > 0) {
            $_SESSION['mensaje'] = "Se insertó el médico con éxito";
        } else {
            $_SESSION['mensaje'] = "Error al grabar el médico";
    ...
    public function documentpdf($cita)
    {
        $gestorCita = new GestorCita();
        $result = $gestorCita->consultarCitaPorId($cita);
